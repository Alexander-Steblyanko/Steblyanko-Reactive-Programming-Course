...
public class Task {
	...
	public static Flux<String> fizzBuzz(Flux<Integer> input) {
		return input.map(i -> new IndexedWord(i, ""))
				.map(iw -> iw.getIndex() % 3 == 0
						? new IndexedWord(iw.getIndex(), iw.getWord() + "Fizz")
						: iw)
				.map(iw -> iw.getIndex() % 5 == 0
						? new IndexedWord(iw.getIndex(), iw.getWord() + "Buzz")
						: iw)
				.map(iw -> iw.getWord().isEmpty()
						? iw.getIndex()+""
						: iw.getWord());
	}
